...
vi.mock("pi-lean-host/core/transport.js", async () => ({
	...(await vi.importActual<typeof import("pi-lean-host/core/transport.js")>(
		"pi-lean-host/core/transport.js",
	)),
	fetchUrl: vi.fn(),
}));
...
async function setupRecipe(): Promise<{ op: Operation; guide: ApiGuide }> {
	...
	const domainDir = join(guidesDir, "wayback-cdx-server");
	...
	const guide = loaded.guides["wayback-cdx-server"]!;
...
describe("queryCdx transform through the real pipeline (mocked transport)", () => {
	it("returns the zipped objects from the CDX array-of-arrays", async () => {
		const { fetchUrl } = await import("pi-lean-host/core/transport.js");
		...
		const transformFn = await loadTransform("wayback-cdx-server");
